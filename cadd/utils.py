...
def proximo_periodo(periodos):
    ...
    periodo_corrente = periodo_atual()
    ano = periodo_corrente[0]
    semestre = periodo_corrente[1] + periodos
    if semestre % 2 == 0:
        periodo = 2
        ano = ano + (semestre // 2) - 1
    else:
        periodo = 1
        ano = ano + (semestre // 2)
    retorno = ano, periodo, str(ano) + "." + str(periodo)
...
def vida_academica(id_aluno):
    ...
    retorno = disciplinas_concluidas, \
              ...
              integralizacao_vermelha

    return retorno


def min_periodos(id_aluno):
    """
        Função que retorna o minimo de períodos a serem cursados pelo aluno logado
    """

    aluno = Aluno.objects.using('sca').get(id=id_aluno)
    versaocurso = Versaocurso.objects.using('sca').get(id=aluno.versaocurso_id)
    minperiodos = versaocurso.qtdperiodominimo
